...
export default class Graph extends React.Component<GraphProps> {
    ...
    private generateGraph() {
      ...
      for (let [_sheetName, sheet] of Object.entries(graphMeta.tacoPatterns)) {
      ...
      if (true) {
        ...
        for (let row of sortedRows) {
          ...
          if (true && horizontalEdges.has(row) && rowsToRange.get(row).length > 2) {
        ...
        for (let col of sortedCols) {
          ...
          if (true && verticalEdges.has(col) && colsToRange.get(col).length > 2) {
    ...
    public render(){
      ...
      for (let i = 0; i < elements.length; i++) {
        ...
        if (position) {
          ...
          elements[i].position.y = position.y * this.props.scale * .5;
        }
      }
      ...
      return <>
        <button onClick={() => this.setScale(20)}>
          set scale to 50
        </button>
        <CytoscapeComponent 
          elements={elements} 
